...
void loop()                                                  //процедура бесконечного цикла
{
  ...
  if(currentTime >= (loopTime + delayButtonRequest)){   // сравниваем текущий таймер с переменной loopTime + 0,1 секунда для задержки в 0,1 секунду
    i_prom++;
    if(i_prom = 10){                                    //будет выполняться в 20 раза реже delayScreenLight (2 секунды) чтобы датчики успевали отработать
    ...
    if(i_prom = 20){                                    //будет выполняться в 20 раза реже delayScreenLight (2 секунды) чтобы датчики успевали отработать
...
// переопределить поведение
  ...
  if(b_updateButtons){                      //время выяснить значение кнопок
  ...
  if(b_updateSensorValue){                         //время обновить значения сенсоров
  ...
  if(b_updateScreen){                             //время обновить экран
    display.clearDisplay();
...
void testdrawbitmap(const uint8_t *bitmap, uint8_t w, uint8_t h) {    //в будущем можно сделать экранную заставку, если температура не трогалась более 5 минут
